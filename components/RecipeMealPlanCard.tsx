...
interface RecipeMealPlanCard {
  ...
  const RecipeMealPlanCard = ({ nameRecipe, preparationTime, image }: RecipeMealPlanCard) => {
    return (
      <div className="recipe-card bg-white dark:bg-slate-600 shadow-md rounded-md p-2 flex items-center mb-4">
        <div className="recipe-info flex-grow p-2">
          <h4 className="text-md font-semibold">{nameRecipe}</h4>
          <p className="text-sm text-gray-600 dark:text-white flex items-center"> <Clock className='pr-2'/> {preparationTime} min</p>
        </div>
